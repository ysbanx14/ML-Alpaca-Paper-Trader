...
| **Mandatory Warning String** | UI explicitly states: *"⚠️ PAPER TRADING ONLY - NO REAL MONEY. This will execute an API call to Alpaca Paper Trading."* |

---

## 🏗️ System Architecture & Modular File Overview

The codebase is strictly modularized into distinct operational layers:

1. **`main_3.py` (The Execution Terminal)**
   - The Streamlit front-end featuring a dynamic, multi-view layout: **"Research & Deploy"** for backtesting, and **"Live Portfolio Tracking"** for monitoring active deployments.
...
6. **`portfolio_manager.py` (State Preservation Layer)**
   - Utilizes a local JSON ledger (`tracked_tickers.json`) to persist deployed models across browser refreshes and app restarts.
...
```bash
pip install streamlit pandas numpy scikit-learn plotly alpaca-py python-dotenv
```
...
4. If the Out-Of-Sample performance is satisfactory, scroll to Section 3 and click **🟢 Deploy ML Strategy** to permanently save the ticker to your live portfolio state.
...
Under the **Deployed Models** list in the sidebar, click the global **⚡ Execute Daily Trades** button. The app will loop through every saved ticker, fetch the latest data, train the model, predict today's signal, and automatically execute the paper trades for your entire portfolio in one click.
